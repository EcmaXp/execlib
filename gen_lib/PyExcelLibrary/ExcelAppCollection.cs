...
using ExcelApplication = PyExcelLibrary.Application;
using System.Diagnostics;
using System.Collections;
using Python.Runtime;

namespace PyExcelLibrary.ExcelExtensions
{
    ...
    /// </summary>
    public class ExcelAppCollection : IEnumerable<ExcelApplication>
    {
        static ExcelAppCollection()
        {
            using (Py.GIL()) { }
        }
        ...
        public ExcelApplication FromProcess(Process process)
        {
            if (process == null)
        ...
        public ExcelApplication FromProcessID(Int32 processID)
        {
            try
            {
        ...
        public ExcelApplication FromMainWindowHandle(Int32 mainHandle)
        {
        ...
        public ExcelApplication PrimaryInstance
        {
            get
            {
                try
                {
                    return new ExcelApplication(Marshal.GetActiveObject(MarshalName) as xlApp);
                }
                catch (COMException)
                {
                    return null;
        ...
        public ExcelApplication TopMostInstance
        {
            get
            {
        ...
        /// </returns>
        public IEnumerator<ExcelApplication> GetEnumerator()
        {
        ...
        private static ExcelApplication InnerFromProcess(Process p)
        {
        ...
        private static ExcelApplication InnerFromHandle(Int32 handle)
        {
            xlWin win = null;
            Int32 hr = AccessibleObjectFromWindow(handle, DW_OBJECTID, rrid.ToByteArray(), ref win);
            return new ExcelApplication(win.Application);
        }
